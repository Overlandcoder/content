...
`<textarea>` also takes a closing tag, and any default text you want it to contain should be put between the opening and closing tags. In contrast, the {{HTMLElement("input")}} is an empty element with no closing tag — and any default value is put inside the [`value`](/en-US/docs/Web/HTML/Attributes/value) attribute.
